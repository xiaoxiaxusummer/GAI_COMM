...
val_loader = DataLoader(val_dataset, batch_size=kept_samples, shuffle=True, num_workers=0, drop_last=True)
...
# alpha_range = np.asarray(args.pilot_alpha)

# Results logging
nmse_all = np.zeros((len(snr_range), kept_samples))
result_dir = './results/ml_baseline/train%s_test%s' % (
