...
def loadChannels(config, seed):
    ...
    for scenario in config.scenario_list:
        fileName = os.path.join(PARENT_DIR,
                                f'DeepMIMO-5GNR/DeepMIMO_dataset/{scenario}_path{config.num_paths}_seed{seed}.mat')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', type=int, default=2)  # Assign the gpu device idx
    parser.add_argument('--train', type=str, default='mixed')
    parser.add_argument('--scenario_list', type=list, default=['O1_28','O1_28B','I2_28B'])
    parser.add_argument('--num_paths', type=list, default=10)
    config = parser.parse_args()

    torch.cuda.set_device(config.gpu)
    model_dir = 'models/wgan_gp/%s/' % config.train
    ...
    # mu_real = np.mean(H_extracted_real,axis=0)
    # mu_imag = np.mean(H_extracted_imag,axis=0)
    # std_real = np.std(H_extracted_real,axis=0)
    # std_imag = np.std(H_extracted_imag,axis=0)
    #
    ...
    for it in range(60000):
        ...
        reset_grad()
        if it%2000 == 0:
            torch.save(G.state_dict(), model_dir + '/generator%d.pt'%it)
