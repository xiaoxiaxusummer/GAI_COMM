...
function params = ConfigDeepMIMOParams(scenario)
%Ray-tracing scenario
params.scenario = scenario;          % The adopted ray tracing scenario [check the available scenarios at https://deepmimo.net/scenarios/]
...
if strcmp(params.scenario,'O1_28B')
    params.active_BS = [3]; 
else
...
% % Active users and subsampling of active users
...
params.row_subsampling = 1;         % Randomly select round(row_subsampling*(active_user_last-params.active_user_first+1)) rows
params.active_user_first = 1;
params.active_user_last = 51;
num_activated_users = 100;
if strcmp(params.scenario,'O1_28B') || strcmp(params.scenario,'O1_28')
%     params.active_user_last = 2751;
    number_of_users_in_row = 181;
elseif strcmp(params.scenario,'I2_28B')
%     params.active_user_last = 701;
    number_of_users_in_row = 201;
elseif strcmp(params.scenario,'Boston5G_28')
%     params.active_user_last = 811;
    number_of_users_in_row = 595;
end
params.user_subsampling = num_activated_users/number_of_users_in_row;
...
params.OFDM_limit = 8;             % Only the first params.OFDM_limit subcarriers will be considered when constructing the channels
...
params.CDL_5G.ueAntSize = [2, 4];  % Number of rows and columns in the rectangular UE array


params.CDL_5G.polarization = 1;    % Polarization
...
if params.CDL_5G.customAntenna
    ...
    params.CDL_5G.bsCustomAntenna = phased.URA('Size', [8, 8], 'ElementSpacing', 0.5*lambda*[1, 1]);
    ...
    params.CDL_5G.ueCustomAntenna = phased.URA('Size', [4, 4], 'ElementSpacing', 0.5*lambda*[1, 1]);
end
